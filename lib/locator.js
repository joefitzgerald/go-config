...
class Locator {
  ...
  // Public: Get the runtime(s) for the given project.
  ...
  // Public: Get the gopath for the given project.
  // Returns the GOPATH if it exists, or false if it is not defined.
  gopath (project, options) {
  ...
  findTool (name, project, options) {
    ...
    return Promise.resolve(null).then(() => {
      ...
      return this.runtime(project).then((runtime) => {
        if (isFalsy(runtime)) {
          return false
        }
  ...
  // Internal: Get a copy of the (potentially patched) environment.
